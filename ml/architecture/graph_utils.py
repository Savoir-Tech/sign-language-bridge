"""
Graph utilities for spatial-temporal graph convolution.

Provides adjacency matrix construction with support for uniform, distance,
and spatial partitioning strategies as described in the ST-GCN paper
...
class GraphWithPartition:
    """Skeleton graph with spatial partitioning for ST-GCN.

    Args:
        num_nodes: Number of spatial nodes in the graph.
        center: Index of the center node.
        inward_edges: List of [child, parent] edges defining the skeleton.
        strategy: Partitioning strategy — ``"uniform"``, ``"distance"``,
            or ``"spatial"`` (default).
        max_hop: Maximum distance between connected nodes.
        dilation: Spacing between kernel points.
    """

    def __init__(self, num_nodes, center, inward_edges,
                 strategy="spatial", max_hop=1, dilation=1):
    ...
    def _build_adjacency(self, strategy):
        ...
        if strategy == "uniform":
            ...
            raise ValueError(f"Unsupported graph partitioning strategy: {strategy}")
